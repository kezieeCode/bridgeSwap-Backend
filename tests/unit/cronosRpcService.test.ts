...
import { cronosRpcService } from '../../src/services/cronosRpcService';

const mockedFetch = fetch as unknown as jest.Mock;

describe('cronosRpcService', () => {
  beforeEach(() => {
    mockedFetch.mockReset();
  });

  it('retrieves cronos balance', async () => {
    ...
    const balance = await cronosRpcService.getBalance('0x0000000000000000000000000000000000000000');
    expect(mockedFetch).toHaveBeenCalledWith('https://evm.cronos.org', expect.any(Object));
    expect(balance).toBe('0x789');
  });
});
